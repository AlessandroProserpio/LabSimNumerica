...
double external_potential(double val)
{
	...
	double k1=1., k2=-2.5;
	return k1*pow(val, 4)+k2*val*val;
}
...
double external_potential_prime(double val)
{
	...
	double k1=1., k2=-2.5;
	return 4.*k1*pow(val, 3)+2.*k2*val;
}
...
double variationalWaveFunction(double v)
{
	...
	//LC of gaussians
	double mu=0.827097, sigma=0.62716;
...
double variationalWaveFunction_second(double v)
{
	...
	//LC of gaussians
	double mu=0.827097, sigma=0.62716;
	double e1=pow((v-mu)/sigma, 2), e2=pow((v+mu)/sigma, 2);
	return (-variationalWaveFunction(v)+e1*exp(-0.5*e1)+e2*exp(-0.5*e2))/(sigma*sigma);
}
...
// (-hbar*hbar/2m)d^2/dx^2 G(x,x',dtau)
...
// (-hbar*hbar/2m)(d^2/dx^2 G(x,x',dtau))/G(x,x',dtau)
