...
class function{
	...
	//////////////////// Elementary Functions of Real Analysis (up to translations and scaling factors) //////////////////
...
class cosine : public function{
	...
	double m_a, m_b, m_c, m_d;
		...
		public:
		// constructor
		cosine(double a, double b, double c, double d);
		// methods to access private members
		void SetParameter(char c, double p);
		double GetParameter(char c) const;
		// virtual methods
		virtual double operator()(double x) const { return m_a*cos(m_b*x+m_c)+m_d; };
};
